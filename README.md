...
*   **斜線指令 (Slash Commands)**：
    ...
    *   `/coin`：丟硬幣 (正面/反面)。
    *   `/me [動作]`：傳送動作訊息。
    *   `/poll [問題] [選項1] [選項2] ...`：發起即時投票。
    *   `/bomb [秒數] [內容]`：發送自毀訊息，時間到自動爆炸消失 (預設 10 秒)。
    *   `/weather [城市]`：查詢即時天氣 (例如：`/weather 台北`、`/weather Tokyo`)，支援兩字中文自動修正 (如：台北 -> 台北市)。
*   **程式碼即時預覽 (Live Code Playground)**：
    *   貼上 `html`或 `javascript` 的程式碼區塊 (Markdown)，會自動出現「▶️ 執行/預覽」按鈕，點擊即可開啟沙盒預覽執行結果。
*   **表情回應 (Reactions)**：滑鼠移到訊息上點擊「☺」，可對訊息按讚 (👍, ❤️, 😂...)，支援即時計數。
